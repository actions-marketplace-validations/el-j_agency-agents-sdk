...
describe('resolveSourceRoot', () => {
  ...
  beforeAll(() => {
    sourceRepoDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agency-agents-sdk-test-src-'));
  ...
  it('clones the source repo into the cache dir', () => {
    cacheDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agency-agents-sdk-test-cache-'));
    _resetSourceMemo();
    const dir = resolveSourceRoot({ repo: sourceRepoDir, ref: 'main', cacheDir });
  ...
  it('memoizes within a single process — a second call does not touch git again', () => {
    cacheDir = fs.mkdtempSync(path.join(os.tmpdir(), 'agency-agents-sdk-test-cache-'));
    _resetSourceMemo();
    const first = resolveSourceRoot({ repo: sourceRepoDir, ref: 'main', cacheDir });
    ...
    const second = resolveSourceRoot({ repo: sourceRepoDir, ref: 'main', cacheDir });
