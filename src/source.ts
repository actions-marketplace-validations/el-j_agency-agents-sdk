...
export const DEFAULT_SOURCE_REPO = 'https://github.com/msitarzewski/agency-agents.git';
...
export function resolveSourceRoot(options: SourceOptions = {}): string {
  const repo = options.repo ?? process.env['AGENCY_AGENTS_SOURCE_REPO'] ?? DEFAULT_SOURCE_REPO;
  const ref = options.ref ?? process.env['AGENCY_AGENTS_SOURCE_REF'] ?? DEFAULT_SOURCE_REF;
  const memoKey = `${repo}#${ref}#${options.cacheDir ?? ''}`;

  const memoized = _resolved.get(memoKey);
  if (memoized !== undefined) return memoized;

  const dir = cacheDirFor(repo, ref, options.cacheDir ?? process.env['AGENCY_AGENTS_SOURCE_CACHE']);

  if (fs.existsSync(path.join(dir, '.git'))) {
    try {
      execFileSync('git', ['-C', dir, 'fetch', '--depth', '1', 'origin', ref], { stdio: 'ignore' });
  ...
  execFileSync('git', ['clone', '--depth', '1', '--branch', ref, repo, dir], { stdio: 'ignore' });
  _resolved.set(memoKey, dir);
  return dir;
}
